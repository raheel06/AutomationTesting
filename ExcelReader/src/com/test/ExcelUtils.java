...
public class ExcelUtils {
	 ...
	 * Result, int RowNum, int ColNum,String FileName) throws Exception { try{
	 * 
	 * 
	 * Row = ExcelWSheet.getRow(RowNum); Cell = Row.getCell(ColNum); if (Cell ==
	 * null) { Cell = Row.createCell(ColNum); Cell.setCellValue(Result); } else {
	 * Cell.setCellValue(Result); } // Constant variables Test Data path and Test
	 * Data file name FileOutputStream fileOut = new
	 * FileOutputStream(Constant_EP.Path_TestData + FileName);
	 * ExcelWBook.write(fileOut); // fileOut.flush(); fileOut.close();
	 * 
	 * // Reload the workbook, workaround for bug 49940 //
	 * https://issues.apache.org/bugzilla/show_bug.cgi?id=49940 //ExcelWBook = new
	 * XSSFWorkbook(new FileInputStream(Constant.Path_TestData +
	 * Constant.File_TestData)); }catch(Exception e){ throw (e); } }
	 */
	 ...
	 * 
	 * FileOutputStream fileOut = new FileOutputStream(Constant_EP.Path_TestData +
	 * FileName); ExcelWBook.write(fileOut); // fileOut.flush(); fileOut.close(); }
	 */
